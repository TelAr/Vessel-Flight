...
public class EnemySpawnerLagacy : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Vector3[] vertices = variations[1].vertices;
        ...
        variations[1].vertices = vertices;
        variations[1].RecalculateBounds();
    ...
    void Spawn(float x, float y, float rotation, EnemyMovement movement, float moveSpeed, EnemyFireType fireType, float startDelay, float fireDelay, float bulletSpeed) {
        ...
        for (int t = 0; t < gameObject.transform.childCount; t++)
        {
            ...
            if (!enemy.activeSelf) {
                ...
                enemy.GetComponent<MeshFilter>().mesh = variations[meshType];
